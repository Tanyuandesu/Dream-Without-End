...
public static class EnemyEA3AlgorithmAudit
{
    ...
    public static void RunAudit()
    {
        ...
        report.AppendLine("CasesPassed=" + passedCases + "/10");
        ...
        report.AppendLine("HybridBoundaryPolicy=CommittedDoorSocketsOnly");

        if (errors.Count == 0 && passedCases == 10)
        {
    ...
    private static void RunHybridWallBoundaryCases(
        EnemyPathService service,
        List<string> errors,
        ref int passedCases)
    {
        GameObject firstRoomObject = null;
        GameObject secondRoomObject = null;

        try
        {
            DreamRoomTemplate firstTemplate =
                CreateAuditRoomTemplate(
                    "EA3_WallAudit_RoomA",
                    "Door_East_A",
            ...
            DungeonLayout hybridLayout = DungeonLayout.CreateHybrid(
                new[] { firstPlacement, secondPlacement },
                corridorCells,
                new[] { connection },
                new Vector2Int(7, 0),
                new Vector2Int(10, 1),
            ...
            EnemyPathResult wallDetour =
                service.FindPathImmediate(
                    new Vector2(7f, 0f),
                    new Vector2(8f, 0f));

            bool crossedCommittedDoor =
                wallDetour.Success &&
                wallDetour.PathCost == 30 &&
                wallDetour.CellPath.Count == 3 &&
                wallDetour.CellPath[0] ==
                    new Vector2Int(7, 1) &&
                wallDetour.CellPath[1] ==
                    new Vector2Int(8, 1) &&
                wallDetour.CellPath[2] ==
                    new Vector2Int(8, 0);

            ValidateCase(
                "Room-wall adjacency detours through committed door",
                crossedCommittedDoor,
                wallDetour,
                errors,
                ref passedCases);

            bool topologyMetadataValid =
                service.UsesHybridTraversalEdges &&
                service.OpenDoorTransitionCount == 4 &&
                service.BlockedRoomBoundaryTransitionCount >= 1 &&
                service.ConnectedComponentCount == 1;

            ValidateCase(
                "Hybrid door-edge topology metadata",
                topologyMetadataValid,
                wallDetour,
                errors,
                ref passedCases);
        }
        finally
        {
            ...
            {
                Object.DestroyImmediate(firstRoomObject);
            }

            if (secondRoomObject != null)
            {
                Object.DestroyImmediate(secondRoomObject);
            }
        }
    }

    private static DreamRoomTemplate CreateAuditRoomTemplate(
        string objectName,
        string socketId,
        DreamRoomDoorDirection direction,
        Vector2Int localInsideCell,
        out GameObject roomObject)
    {
        roomObject = new GameObject(objectName);
        roomObject.hideFlags = HideFlags.HideAndDontSave;

        GameObject socketObject = new GameObject(socketId);
        socketObject.hideFlags = HideFlags.HideAndDontSave;
        socketObject.transform.SetParent(roomObject.transform);
        ...
        DreamRoomTemplate template =
            roomObject.AddComponent<DreamRoomTemplate>();
