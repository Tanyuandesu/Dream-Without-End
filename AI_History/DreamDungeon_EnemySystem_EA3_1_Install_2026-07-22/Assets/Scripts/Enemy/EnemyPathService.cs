...
public sealed class EnemyPathService : MonoBehaviour
{
    ...
    [SerializeField] private bool usesHybridTraversalEdges;
    [SerializeField] private int roomOwnedCellCount;
    [SerializeField] private int openDoorTransitionCount;
    [SerializeField] private int blockedRoomBoundaryTransitionCount;
    ...
    private readonly Dictionary<Vector2Int, int> roomOwnerByCell =
        new Dictionary<Vector2Int, int>();

    private readonly HashSet<UndirectedCellEdge> openDoorTransitions =
        new HashSet<UndirectedCellEdge>();
    ...
    public bool UsesHybridTraversalEdges => usesHybridTraversalEdges;
    public int RoomOwnedCellCount => roomOwnedCellCount;
    public int OpenDoorTransitionCount => openDoorTransitionCount;
    public int BlockedRoomBoundaryTransitionCount =>
        blockedRoomBoundaryTransitionCount;
        ...
        bool newSimplifyCollinearWaypoints)
    {
        ...
        roomOwnerByCell.Clear();
        openDoorTransitions.Clear();
        ...
        BuildHybridTraversalEdges(layout);
        ...
        ref bool prunedByPathCost)
    {
        for (int i = 0; i < CardinalDirections.Length; i++)
        {
            ...
            if (!CanTraverseCardinal(current, neighbour))
            {
                continue;
            }
        ...
        Vector2Int diagonalDirection)
    {
        ...
        Vector2Int diagonal = current + diagonalDirection;

        return floorCells.Contains(diagonal) &&
               floorCells.Contains(horizontal) &&
               floorCells.Contains(vertical) &&
               CanTraverseCardinal(current, horizontal) &&
               CanTraverseCardinal(current, vertical) &&
               CanTraverseCardinal(horizontal, diagonal) &&
               CanTraverseCardinal(vertical, diagonal);
    }

    /// <summary>
    /// FloorCells describe occupied walkable cells, but a hybrid room wall is
    /// an edge between two otherwise walkable cells. A corridor may therefore
    /// run beside a room while remaining physically separated by the prefab
    /// wall. Runtime traversal must cross a room boundary only through a
    /// socket selected by a committed DreamRoomConnection.
    /// </summary>
    private void BuildHybridTraversalEdges(DungeonLayout layout)
    {
        usesHybridTraversalEdges =
            layout != null &&
            layout.HasHybridRoomData &&
            layout.RoomPlacements != null &&
            layout.RoomPlacements.Count > 0;

        roomOwnedCellCount = 0;
        openDoorTransitionCount = 0;
        blockedRoomBoundaryTransitionCount = 0;

        if (!usesHybridTraversalEdges)
        {
            return;
        }
             ...
             roomIndex++)
        {
            ...
            placement.GetWalkableGlobalCells(placementCells);

            for (int cellIndex = 0;
                 cellIndex < placementCells.Count;
                 cellIndex++)
            {
                Vector2Int cell = placementCells[cellIndex];

                if (floorCells.Contains(cell) &&
                    !roomOwnerByCell.ContainsKey(cell))
                {
                    roomOwnerByCell.Add(cell, roomIndex);
                }
            }
        }

        roomOwnedCellCount = roomOwnerByCell.Count;
        ...
        openDoorTransitionCount = openDoorTransitions.Count;
        blockedRoomBoundaryTransitionCount =
            CountBlockedRoomBoundaryTransitions();
    }

    private void AddOpenSocketTransitions(
        DungeonLayout layout,
        int roomIndex,
        string socketId)
    {
        ...
        placement.GetSocketInsideCells(socket, insideCells);
        placement.GetSocketOutsideCells(socket, outsideCells);
        ...
        {
            Vector2Int inside = insideCells[i];
            Vector2Int outside = outsideCells[i];

            if (!floorCells.Contains(inside) ||
                !floorCells.Contains(outside) ||
                !roomOwnerByCell.TryGetValue(
                    inside,
                    out int insideRoomIndex) ||
                insideRoomIndex != roomIndex ||
                roomOwnerByCell.ContainsKey(outside))
            {
                continue;
            }

            openDoorTransitions.Add(
                new UndirectedCellEdge(inside, outside));
        }
    }
    ...
    private bool CanTraverseCardinal(
        ...
        Vector2Int second)
    {
        if (!floorCells.Contains(first) ||
            !floorCells.Contains(second))
        {
            return false;
        }
        ...
        bool firstIsRoom = roomOwnerByCell.TryGetValue(
            first,
            out int firstRoomIndex);

        bool secondIsRoom = roomOwnerByCell.TryGetValue(
            second,
            out int secondRoomIndex);

        if (!firstIsRoom && !secondIsRoom)
        {
            return true;
        }
        ...
        return openDoorTransitions.Contains(
            new UndirectedCellEdge(first, second));
    }

    private int CountBlockedRoomBoundaryTransitions()
    {
        int count = 0;
        Vector2Int[] uniqueDirections =
        {
            Vector2Int.right,
            Vector2Int.up
        };
        ...
        {
            for (int i = 0; i < uniqueDirections.Length; i++)
            {
                Vector2Int neighbour =
                    cell + uniqueDirections[i];

                if (floorCells.Contains(neighbour) &&
                    !CanTraverseCardinal(cell, neighbour))
                {
                    count++;
                }
            }
    ...
    private readonly struct UndirectedCellEdge :
        IEquatable<UndirectedCellEdge>
    {
        private readonly Vector2Int first;
        private readonly Vector2Int second;

        public UndirectedCellEdge(
            Vector2Int firstCell,
            Vector2Int secondCell)
        {
            if (CompareCells(firstCell, secondCell) <= 0)
            {
                first = firstCell;
                second = secondCell;
            }
        ...
        public bool Equals(UndirectedCellEdge other)
        {
        ...
        {
            return obj is UndirectedCellEdge other &&
                   Equals(other);
        }
