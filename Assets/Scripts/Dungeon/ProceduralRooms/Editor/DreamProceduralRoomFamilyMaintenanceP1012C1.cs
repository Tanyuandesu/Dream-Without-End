...
/// </summary>
public static class DreamRoomFidelityRuntimeCompositionP10133
{
    private const string MenuRoot =
        "Tools/Dream Dungeon/Room Mix/P10.13-3 Runtime Composition/";

    private const string LastSoakPassKey =
        "DreamDungeon.P10.13.3.RuntimeCompositionSoakPassed";

    private const string LastSoakSummaryKey =
        "DreamDungeon.P10.13.3.RuntimeCompositionSoakSummary";
    ...
    [MenuItem(
        MenuRoot + "1. Validate LIVE Current Composition",
        false,
        3400)]
    private static void ValidateCurrentComposition()
    {
        ...
        {
            Debug.LogError(
                "[P10.13-3] Current Composition Audit 必须在 Play Mode。");
            return;
        }
        ...
        {
            Debug.LogError(
                "[P10.13-3] 找不到 GameManager / DungeonGenerator。");
            return;
        }
    ...
    [MenuItem(
        MenuRoot + "2. Start 24-Case Runtime Composition Soak",
        false,
        3410)]
    private static void StartRuntimeSoak()
    {
        ...
        {
            Debug.LogError(
                "[P10.13-3] Runtime Composition Soak 必须在 Play Mode。");
            return;
        }
        ...
        {
            Debug.LogWarning(
                "[P10.13-3] Runtime Composition Soak 已经在运行。");
            return;
        }
        ...
        {
            Debug.LogError(
                "[P10.13-3] Reflection Contract 不完整。" +
                "\ncurrentLayout=" +
                (currentLayoutField != null) +
                " | useRandomSeed=" +
                (useRandomSeedField != null) +
        ...
        Debug.Log(
            "[P10.13-3] Runtime Composition Soak START" +
            "\nCases=" +
            TotalCases +
            " | Floors=1,2,3" +
            " | SeedsPerFloor=" +
    ...
    [MenuItem(
        MenuRoot + "3. Abort Running Composition Soak",
        false,
        3420)]
    private static void AbortRuntimeSoak()
    {
        ...
        {
            Debug.Log(
                "[P10.13-3] 当前没有正在运行的 Composition Soak。");
            return;
        }
    ...
    [MenuItem(
        MenuRoot + "4. Print Last Composition Soak Summary",
        false,
        3430)]
    private static void PrintLastSoakSummary()
    {
        ...
        {
            Debug.Log(
                "[P10.13-3] 尚无 Runtime Composition Soak 结果。");
            return;
        }
        ...
        bool pass)
    {
        ...
        builder.AppendLine(
            "[P10.13-3] LIVE Fidelity Runtime Composition Audit " +
            (pass ? "PASS" : "FAILED"));
        ...
        string reason)
    {
        ...
        builder.AppendLine(
            "[P10.13-3] Runtime Fidelity Composition Soak " +
            (pass ? "PASS" : "FAILED"));
