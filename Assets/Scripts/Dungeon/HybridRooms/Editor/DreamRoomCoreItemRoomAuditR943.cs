...
/// <summary>
/// R9.4.3 Core Item 候选房与 ItemSpawner 房间作用域的
/// 静态资产、纯数据和真实运行验收工具。
///
...
/// Catalog 与固定 Seed。
/// </summary>
public static class DreamRoomCoreItemRoomAuditR943
{
    ...
    private const string RuntimeCatalogPath =
        TestRoot + "/Catalog/RoomCatalog_R943_Runtime.asset";

    private const string RuntimeCatalogId =
        "CoreItemRooms_R943_Runtime";
    ...
    private static readonly TemplateSpec[] RuntimeTemplateSpecs =
    {
        ...
        new TemplateSpec(
            RoomRoot + "/Room_R943_Exit.prefab",
            ExitTemplateId,
            DreamRoomTag.Standard |
        ...
        new TemplateSpec(
            RoomRoot + "/Room_R943_Common.prefab",
            CommonTemplateId,
            DreamRoomTag.Standard,
            randomWeight: 4,
        ...
        new TemplateSpec(
            RoomRoot + "/Room_R943_Rare.prefab",
            RareTemplateId,
            DreamRoomTag.Rare,
            randomWeight: 1,
            maximumInstancesPerFloor: 1),
        ...
        new TemplateSpec(
            RoomRoot + "/Room_R943_Core.prefab",
            CoreTemplateId,
        ...
        new TemplateSpec(
            RoomRoot + "/Room_R943_CoreSpecialDecoy.prefab",
        ...
        new TemplateSpec(
            RoomRoot + "/Room_R943_SpecialDeferred.prefab",
    ...
    [MenuItem(
        MenuRoot +
        "Validate Installed Core Item Test Assets (R9.4.3)",
        false,
        2520)]
    private static void ValidateInstalledAssets()
    {
        ...
        if (catalog != null)
        {
            ...
            for (int i = 0;
                 i < RuntimeTemplateSpecs.Length;
                 i++)
            {
                socketOwners +=
                    ValidateStaticPrefab(
                        RuntimeTemplateSpecs[i],
                        errors);
            }
        }
        ...
        {
            errors.Add(
                "只读校验改变了 GameScene／既有基线／R9.4.3 测试资产哈希。" );
        }
        ...
        Debug.Log(
            "[DreamRoomCoreItemRoomAuditR943] " +
            "R9.4.3 静态测试资产校验通过。\n" +
            ...
            "/" +
            (RuntimeTemplateSpecs.Length *
             ExpectedSocketCountPerTemplate) + "\n" +
            "CoreCandidate=" + CoreTemplateId +
            ":Weight1:Cap1" +
            " | CoreSpecialDecoy=" +
            CoreSpecialDecoyTemplateId +
            ":Weight1000:Deferred" +
        ...
        EditorUtility.DisplayDialog(
            "R9.4.3 Assets Passed",
            "7 个静态模板、28/28 个 Socket Owner、" +
            "Core 与 Special 反向诱饵均已通过。",
            "OK");
    }

    [MenuItem(
        MenuRoot +
        "Validate Core Item Room-Scope Contract (R9.4.3)",
        false,
        2530)]
    private static void ValidateCoreItemContract()
    {
        SceneContext context;
        List<string> errors;
    ...
    [MenuItem(
        MenuRoot +
        "Prepare Core Item Runtime Test (R9.4.3)",
        false,
        2540)]
    private static void PrepareRuntimeTest()
    {
        ...
        Debug.Log(
            "[DreamRoomCoreItemRoomAuditR943] " +
            "R9.4.3 Core Item Runtime Test 已准备。\n" +
            "Catalog=" + RuntimeCatalogId +
            " | FixedSeed=" + liveBaseSeed +
        ...
        EditorUtility.DisplayDialog(
            "R9.4.3 Runtime Test Ready",
            "GameScene 当前只在内存中使用 R9.4.3 Catalog 与测试 Seed。\n\n" +
    ...
    [MenuItem(
        MenuRoot +
        "Advance to Core Item Floor 2 (R9.4.3)",
        false,
        2550)]
    ...
    [MenuItem(
        MenuRoot +
        "Validate Live Core Item Scope (R9.4.3)",
        false,
        2560)]
    private static void ValidateLiveCoreItemScope()
    {
        ...
        {
            ReportFailure(
                "R9.4.3 Live Core Item Scope 失败",
                errors,
                generator);
            return;
        }
        ...
        Debug.Log(
            ...
            " | CoreSpecialDecoyPlacements=0\n" +
            "ActivePickup=" + pickup.name +
            " | ItemCell=" + itemCell +
            " | ItemRoomIndex=" + itemRoomIndex +
            " | ItemInsideCoreRoom=" +
            itemInsideCoreRoom + "\n" +
            "ItemScope=CoreItemCandidateRooms" +
            " | LayoutWideFallbackAllowed=False" +
            " | RuntimeObjectsModified=False",
            generator);

        EditorUtility.DisplayDialog(
            "R9.4.3 Live Core Item Passed",
            "Floor 2 的核心道具已确认生成在唯一 Core 候选房内。",
            "OK");
    }

    [MenuItem(
        MenuRoot +
        "Restore and Save Graybox after R9.4.3",
        false,
        2570)]
    private static void RestoreGrayboxBaseline()
    {
        ...
        Debug.Log(
            ...
            " | R9.4.3TestAssetsRetained=True" +
            " | RuntimePatchRetained=True",
            generator);

        EditorUtility.DisplayDialog(
            "R9.4.3 Graybox Restored",
